...
local BASE_PARRY_COOLDOWN = 0.001          -- praticamente zero
local BASE_PARRY_PRESS_DURATION = 0.15    -- segura mais tempo
local MIN_PARRY_PRESS = 0.10
local MAX_PARRY_PRESS = 0.25
local PARRY_RECOVERY = 0.001              -- recuperação instantânea
local THREAT_SPEED = 5                    -- qualquer movimento
local THREAT_DISTANCE = 15                -- alcance máximo
local PREDICTIVE_PARRY_DISTANCE = 10      -- antecipa de muito longe
local WINDUP_ANIMATION_THRESHOLD = 0.8    -- pega quase toda animação
local ACCELERATION_THRESHOLD = 20         -- detecta acelerações mínimas
...
local enemyStats = {
    ...
    lastPosition = nil,
    playStyle = "unknown",
    lastVelocity = Vector3.zero,          -- para cálculo de aceleração
    lastVelocityTime = 0,
}
...
local detectedCombos = {}         -- combos já vistos (strings)
local learnedComboFunctions = {}  -- combos convertidos em funções
local comboCount = {}             -- contagem de combos

-- Classificação de golpes (agora com fallback para ID)
local function classifyAttack(animId)
    ...
    elseif lower:find("attack") or lower:find("hit") or lower:find("ataque") or lower:find("golpe") then
        ...
        -- Se não reconhecer, retorna o ID como rótulo
...
local attackFunctionMap = {
    ...
    ["Chute"] = "highkick",
    ["Ataque"] = "punch",
    -- Movimentos desconhecidos também mapeados para soco (fallback)
    ["MOVE"] = "punch", -- não usado diretamente, mas para evitar erros
}
        ...
        if not table.find(detectedCombos, comboStr) then
            ...
            for _, attackType in ipairs(currentSequence) do
                ...
                elseif funcName == "lowkick" then func = executeLowKick
                    ...
                    -- Fallback para golpes não mapeados: usa soco
                    func = executePunch
                end
                if func then
