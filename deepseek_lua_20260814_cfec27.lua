-- ================================================================
-- AUTO COMBATE ADAPTATIVO + AUTO PARRY MELHORADO + SPAM TOTAL
-- ================================================================
...
-- Parry (tecla G) - altere aqui se necessário
local PARRY_PRESS = buffer.fromstring("\005\bkeypress\v\001g\000\000\000\000")
...
-- Parry melhorado
local BASE_PARRY_COOLDOWN = 0.08
local BASE_PARRY_PRESS_DURATION = 0.04
local MIN_PARRY_PRESS = 0.02
local MAX_PARRY_PRESS = 0.08
local PARRY_RECOVERY = 0.05
local THREAT_SPEED = 10               -- era 14, agora mais sensível a movimentos rápidos
local THREAT_DISTANCE = 9
local PREDICTIVE_PARRY_DISTANCE = 5
local WINDUP_ANIMATION_THRESHOLD = 0.15  -- detecta ataques logo no início da animação
...
local enemyStats = {
    ...
    lastPosition = nil,
    playStyle = "unknown",
    lastVelocity = Vector3.zero,       -- novo: para calcular aceleração
    lastVelocityTime = 0,
}
...
-- Previsão de fim de defesa inimiga (para chute no fim)
...
local function updateEnemyStats(enemyChar, myRoot)
    ...
    -- Atualizar aceleração (para dash)
    if now - enemyStats.lastVelocityTime > 0.1 then
        local currentVel = targetRoot.Velocity
        if enemyStats.lastVelocity.Magnitude > 0 then
            local acceleration = (currentVel - enemyStats.lastVelocity).Magnitude / (now - enemyStats.lastVelocityTime)
            -- Podemos usar essa aceleração para prever dash, mas por enquanto armazenamos
            -- (não usado diretamente, mas disponível para futuras melhorias)
        end
        enemyStats.lastVelocity = currentVel
...
-- ==================== DECISÃO DE PARRY (MELHORADA) ====================
local function shouldParry(enemyChar, myRoot)
    if not enemyChar or not myRoot then return false end
    local targetRoot = enemyChar:FindFirstChild("HumanoidRootPart") or enemyChar:FindFirstChild("Torso")
    if not targetRoot then return false end

    if tick() < parryRecoveryUntil then
        return false
    end

    local dist = (myRoot.Position - targetRoot.Position).Magnitude

    -- 1. Inimigo está em animação de ataque (wind-up ou ativa)
    if isEnemyAttackingNow(enemyChar) then
        -- Verificar se a animação está no início (wind-up)
        local hum = enemyChar:FindFirstChildOfClass("Humanoid")
        if hum then
            for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                local anim = track.Animation
                if anim and track.IsPlaying then
                    local name = string.lower(anim.AnimationId)
                    if name:find("attack") or name:find("punch") or name:find("kick") or name:find("soco") or name:find("chute") or name:find("hit") then
                        -- Se a animação acabou de começar ou está no início, é o momento ideal
                        if track.TimePosition <= WINDUP_ANIMATION_THRESHOLD and dist <= 10 then
        ...
        -- Se não detectou wind-up, mas distância é curta, ainda tenta parry
        if dist <= 8 then
            enemyStats.lastAttackTime = tick()
            return true
        end
    ...
    -- 2. Movimento rápido em nossa direção (dash)
    ...
    -- 3. Previsão: inimigo próximo, olhando para nós e se movendo
    if dist <= PREDICTIVE_PARRY_DISTANCE then
...
RunService.RenderStepped:Connect(function()
    ...
    -- Solta o parry se o tempo de pressionamento acabou
    ...
    -- Atualizar tempo restante da defesa inimiga (para chute no fim)
    ...
    -- Ajustar parry conforme estilo
    if enemyStats.playStyle == "aggressive" then
        currentParryCooldown = 0.05
        currentParryPressDuration = math.max(MIN_PARRY_PRESS, BASE_PARRY_PRESS_DURATION - 0.01)
    elseif enemyStats.playStyle == "kiter" then
        currentParryCooldown = 0.08
    ...
    if shouldParry(target, myRoot) then
        if not isParrying and (tick() - lastParryTime >= currentParryCooldown) then
            ...
            print("🛡️ Parry ativado! (duração: " .. tostring(currentParryPressDuration) .. "s)")
        end
        -- Sempre que deve fazer parry, retorna para priorizar defesa
        return
    end
...
print("🧠 Auto parry melhorado + spam total de socos e chutes ativados!")