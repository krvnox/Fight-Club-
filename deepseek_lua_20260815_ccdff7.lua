-- ================================================================
-- AUTO COMBATE ADAPTATIVO + AUTO PARRY + SPAM CONTROLADO + ANIMAÇÕES
-- ================================================================
...
local HIGHKICK_PRESS = buffer.fromstring("\005\bkeypress\v\001q\000\000\000\000")
local HIGHKICK_RELEASE = buffer.fromstring("\005\bkeyrelease\v\001q\000\000\000\000")
local LOWKICK_PRESS = buffer.fromstring("\005\bkeypress\v\001e\000\000\000\000")
local LOWKICK_RELEASE = buffer.fromstring("\005\bkeyrelease\v\001e\000\000\000\000")
...
-- Cooldowns de ataque (ajustados para animações)
local PUNCH_COOLDOWN_NORMAL = 0.02    -- soco rápido
local PUNCH_COOLDOWN_FAST = 0.008
local KICK_COOLDOWN_NORMAL = 0.04    -- chute mais lento para animação
...
local function executeHighKick()
    pcall(function()
        packetRemote:FireServer(HIGHKICK_PRESS)
        task.wait(0.03)
        packetRemote:FireServer(HIGHKICK_RELEASE)
    end)
end

local function executeLowKick()
    pcall(function()
        packetRemote:FireServer(LOWKICK_PRESS)
        task.wait(0.03)
        packetRemote:FireServer(LOWKICK_RELEASE)
    end)
end
...
print("🧠 Chutes com animação corrigida! Parry e spam moderado ativados.")