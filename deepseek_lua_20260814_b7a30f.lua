...
local function chooseAdaptiveCombo()
    ...
    for _, funcs in ipairs(pool) do
        ...
        local speedScore = punches
        local powerScore = kicks + lowKicks
        local guardScore = lowKicks

        local score = 0
        if situation == "fast" then
            score = speedScore * 3 + powerScore * 1
        elseif situation == "power" then
            score = powerScore * 3 + speedScore * 1
        elseif situation == "guardbreak" then
            score = guardScore * 3 + powerScore * 1 + speedScore * 0.5
        else
            score = speedScore * 1.5 + powerScore * 1.5
        end
