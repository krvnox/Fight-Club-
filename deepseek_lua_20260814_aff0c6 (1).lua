...
local PARRY_PRESS = buffer.fromstring("\005\bkeypress\v\001g\000\000\000\000")   -- Tecla do parry
...
-- Parry ultra-rápido
local BASE_PARRY_COOLDOWN = 0.02
local BASE_PARRY_PRESS_DURATION = 0.08
local MIN_PARRY_PRESS = 0.04
local MAX_PARRY_PRESS = 0.12
local PARRY_RECOVERY = 0.01
local THREAT_SPEED = 8
local THREAT_DISTANCE = 10
local PREDICTIVE_PARRY_DISTANCE = 7
local WINDUP_ANIMATION_THRESHOLD = 0.5
...
local detectedCombos = {}         -- lista de combos (strings) já vistos
local learnedComboFunctions = {}  -- combos convertidos em funções
local comboCount = {}             -- contagem de combos (CORRIGIDO: agora declarada antes)

-- Classificação de golpes (ajuste conforme as animações do seu jogo)
...
-- Novo: label de última ação detectada
...
-- Função para atualizar a UI do observador (agora usa comboCount que já existe)
local function updateObserverUI()
    ...
    if not comboCount then return end  -- segurança
    ...
    if #currentSequence >= MIN_COMBO_LENGTH then
        ...
        if not table.find(detectedCombos, comboStr) then
            ...
            for _, attackType in ipairs(currentSequence) do
                ...
                elseif funcName == "lowkick" then func = executeLowKick end
                if func then
...
local function shouldParry(enemyChar, myRoot)
    ...
    local dist = (myRoot.Position - targetRoot.Position).Magnitude
    local threatLevel = sharinganActive and 1.5 or 1.0
        ...
        if hum then
            for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                ...
                if anim and track.IsPlaying then
                    local name = string.lower(anim.AnimationId)
                    if name:find("attack") or name:find("punch") or name:find("kick") or name:find("soco") or name:find("chute") or name:find("hit") then
                        if track.TimePosition <= WINDUP_ANIMATION_THRESHOLD and dist <= 12 * threatLevel then
    ...
    if dist <= PREDICTIVE_PARRY_DISTANCE * threatLevel then
        ...
        if facing > 0.2 and not isEnemyBlockingNow(enemyChar) then
            local moveSpeed = vel and vel.Magnitude or 0
            if moveSpeed > 1 then return true end
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if sharinganActive then
        currentParryCooldown = 0.01
        currentParryPressDuration = math.min(MAX_PARRY_PRESS, BASE_PARRY_PRESS_DURATION + 0.04)
...
print("🧠 Auto parry ultra + observador de combos ativados!")