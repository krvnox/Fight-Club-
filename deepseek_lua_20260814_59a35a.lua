...
-- COM SALVAMENTO AUTOMÁTICO (SÓ SALVA COMBOS DE QUEM TE DERROTOU)
-- + SISTEMA ADAPTATIVO DE ESCOLHA DE COMBO
-- ================================================================
...
-- Parry ultra-rápido (nível insano)
...
local detectedCombos = {}         -- combos permanentes (salvos)
local learnedComboFunctions = {}  -- combos permanentes convertidos em funções
local comboCount = {}             -- contagem para exibição (temporários + permanentes)

-- Combos temporários do inimigo atual (não salvos até morrer)
local tempCombosFromCurrentEnemy = {}
...
local function resetComboObserver(newTarget)
    ...
    tempCombosFromCurrentEnemy = {}   -- limpa temporários ao trocar de inimigo
    if newTarget then
...
local function finalizeLearnedCombo()
    if #currentSequence >= MIN_COMBO_LENGTH then
        ...
        -- Apenas registra temporariamente
        if not table.find(tempCombosFromCurrentEnemy, comboStr) then
            table.insert(tempCombosFromCurrentEnemy, comboStr)
            print("🔄 Registrado combo do inimigo (temporário): " .. comboStr)
        end
        updateObserverUI()
    end
    currentSequence = {}
...
local function onPlayerDefeat()
    print("💀 Jogador derrotado! Processando combos do inimigo...")
    if currentTarget then
        for _, comboStr in ipairs(tempCombosFromCurrentEnemy) do
            if not table.find(detectedCombos, comboStr) then
                table.insert(detectedCombos, comboStr)
                -- Converte para funções
                local newCombo = {}
                ...
                print("✅ Combo do derrotador salvo: " .. comboStr)
            end
        end
        saveCombos()
        tempCombosFromCurrentEnemy = {}
    else
        print("❌ Nenhum inimigo registrado no momento da derrota.")
    end
end
...
local function getComboStats(funcs)
    ...
    for _, f in ipairs(funcs) do
        if f == executePunch then
            punches = punches + 1
        elseif f == executeHighKick then
            kicks = kicks + 1
        elseif f == executeLowKick then
            lowKicks = lowKicks + 1
        end
    end
...
local function chooseAdaptiveCombo()
    ...
    local pool = (#learnedComboFunctions > 0) and learnedComboFunctions or fallbackCombos
    local bestCombo = nil
    local bestScore = -9999

    for _, funcs in ipairs(pool) do
        local punches, kicks, lowKicks = getComboStats(funcs)
    ...
    currentCombo = bestCombo or fallbackCombos[1]
    comboStep = 1
    print(("🎯 Situação: %s | Combo selecionado: %s"):format(situation, table.concat(currentCombo, " > ")))
end

-- Inicializa o combo atual com um fallback
currentCombo = fallbackCombos[1]
comboStep = 1

-- Carrega combos salvos no início (após funções definidas)
loadCombos()
...
print("🧠 Auto parry ultra + observador de combos adaptativo com salvamento ativados!")