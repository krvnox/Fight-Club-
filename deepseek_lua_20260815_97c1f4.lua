...
-- COM BUFFERS CORRETOS E COMBOS DE ANIME (ITADORI EM DESTAQUE)
-- ================================================================
...
local comboStep = 1
local currentCombo = {}
local currentComboName = "Itadori"
...
-- ==================== COMBOS DE ANIME (ITADORI EM DESTAQUE) ====================
local animeCombos = {
    {name = "Itadori Yuji (Divergent Fist)", funcs = {executePunch, executePunch, executeHighKick}},
    {name = "Naruto Uzumaki (Rasengan Barrage)", funcs = {executeHighKick, executePunch, executeLowKick}},
    {name = "Goku (Dragon Fist)", funcs = {executeHighKick, executeHighKick, executePunch}},
    {name = "Monkey D. Luffy (Gatling)", funcs = {executePunch, executeLowKick, executeHighKick}},
    {name = "Baki Hanma (Grappler Combo)", funcs = {executeLowKick, executePunch, executeHighKick}},
    {name = "Levi Ackerman (Ackerman Style)", funcs = {executeHighKick, executeLowKick, executePunch}},
    {name = "Saitama (Normal Punch Combo)", funcs = {executePunch, executePunch, executeLowKick}},
    {name = "Tanjiro Kamado (Hinokami Kagura)", funcs = {executeLowKick, executeHighKick, executePunch}},
    {name = "Ichigo Kurosaki (Bankai Combo)", funcs = {executeHighKick, executePunch, executePunch}},
    {name = "Asta (Black Meteorite)", funcs = {executePunch, executeHighKick, executeLowKick}},
}
...
-- ==================== CLASSIFICAÇÃO DE GOLPES (OBSERVADOR) ====================
...
local function saveCombos()
    ...
    local data = {combos = detectedCombos}
    pcall(function()
...
-- ==================== SISTEMA ADAPTATIVO COM COMBOS DE ANIME ====================
local function getComboStats(funcs)
    local punches = 0
    local kicks = 0
    local lowKicks = 0
    for _, f in ipairs(funcs) do
        if f == executePunch then punches = punches + 1
        elseif f == executeHighKick then kicks = kicks + 1
        elseif f == executeLowKick then lowKicks = lowKicks + 1 end
    end
    return punches, kicks, lowKicks
end

local function chooseAdaptiveCombo()
    local myHum = player.Character and player.Character:FindFirstChildOfClass("Humanoid")
    local myHealthRatio = myHum and (myHum.Health / myHum.MaxHealth) or 1
    local targetHum = currentTarget and currentTarget:FindFirstChildOfClass("Humanoid")
    local enemyHealthRatio = targetHum and (targetHum.Health / targetHum.MaxHealth) or 1

    local situation = "balanced"
    if enemyStats.playStyle == "aggressive" or myHealthRatio < 0.4 then
        situation = "fast"
    elseif enemyStats.playStyle == "passive" or enemyHealthRatio < 0.3 then
        situation = "power"
    elseif isEnemyBlockingNow(currentTarget) then
        situation = "guardbreak"
    end

    local pool = {}
    for _, entry in ipairs(animeCombos) do
        table.insert(pool, {funcs = entry.funcs, name = entry.name})
    end
    if #learnedComboFunctions > 0 then
        for _, funcs in ipairs(learnedComboFunctions) do
            table.insert(pool, {funcs = funcs, name = "Aprendido"})
        end
    end
    ...
    for _, entry in ipairs(pool) do
        local punches, kicks, lowKicks = getComboStats(entry.funcs)
        local score = 0
        if situation == "fast" then
            score = punches * 3 + (kicks + lowKicks) * 1
        elseif situation == "power" then
            score = (kicks + lowKicks) * 3 + punches * 1
        elseif situation == "guardbreak" then
            score = lowKicks * 3 + kicks * 1 + punches * 0.5
        else
            score = punches * 1.5 + (kicks + lowKicks) * 1.5
        end

        -- Bônus especial para o Itadori (força extra)
        if entry.name:find("Itadori") then
            score = score + 2  -- prioriza o Itadori
        end
    ...
    currentCombo = bestCombo.funcs or animeCombos[1].funcs
    currentComboName = bestCombo.name or "Itadori"
    comboStep = 1
    print(("🎌 Combo %s selecionado! | Situação: %s"):format(currentComboName, situation))
end

-- Inicializa combo
chooseAdaptiveCombo()
loadCombos()  -- carrega salvos e reescolhe combo
...
print("🎌 Auto combate com Itadori em destaque + buffers corrigidos ativados!")