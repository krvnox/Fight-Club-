-- ================================================================
-- AUTO COMBATE ADAPTATIVO + AUTO PARRY ULTRA (USANDO BLOQUEIO F) 
-- + OBSERVADOR DE COMBOS + BUFFERS REAIS DE ATAQUE
-- ================================================================
...
local PUNCH_TARGET = workspace:WaitForChild("subway"):WaitForChild("concretes"):WaitForChild("concrete")
...
local HIGHKICK_TARGET = workspace:WaitForChild("train"):WaitForChild("train"):WaitForChild("body")
...
local LOWKICK_TARGET = workspace:WaitForChild("railway"):WaitForChild("beam")
...
-- Parry ultra-rápido (agora usando F)
...
-- ==================== FUNÇÕES DE ATAQUE COM NOVOS BUFFERS ====================
function executePunch()
    pcall(function()
        packetRemote:FireServer(PUNCH_BUFFER, {PUNCH_TARGET})
    end)
end

function executeHighKick()
    pcall(function()
        packetRemote:FireServer(HIGHKICK_BUFFER, {HIGHKICK_TARGET})
    end)
end

function executeLowKick()
    pcall(function()
        packetRemote:FireServer(LOWKICK_BUFFER, {LOWKICK_TARGET})
    end)
end
...
-- ==================== DECISÃO DE PARRY (SEM ANIMAÇÕES) ====================
...
print("🧠 Auto parry ultra (bloqueio F) + buffers reais de ataque ativados!")