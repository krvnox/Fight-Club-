...
local BASE_PARRY_COOLDOWN = 0.0            -- zero (reativa imediatamente)
local BASE_PARRY_PRESS_DURATION = 0.2     -- segura por mais tempo
local MIN_PARRY_PRESS = 0.15
local MAX_PARRY_PRESS = 0.3
local PARRY_RECOVERY = 0.0                -- recuperação instantânea
local THREAT_SPEED = 3                    -- qualquer movimento
local THREAT_DISTANCE = 20                -- alcance máximo
local PREDICTIVE_PARRY_DISTANCE = 15      -- antecipa de muito longe
local WINDUP_ANIMATION_THRESHOLD = 1.0    -- pega a animação inteira
local ACCELERATION_THRESHOLD = 10         -- detecta acelerações mínimas
...
local function shouldParry(enemyChar, myRoot)
    ...
    -- 1. Inimigo atacando (qualquer progresso da animação)
    if isEnemyAttackingNow(enemyChar) then
        local hum = enemyChar:FindFirstChildOfClass("Humanoid")
        if hum then
            for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
...
print("🧠 Auto parry ultra melhorado + observador de combos ativados!")